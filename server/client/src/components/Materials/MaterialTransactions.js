...
const MaterialTransactions = (props) => {
	...
	useEffect(() => {
		...
		axios
			.get(`/api/matdocs/${matcode.toString()}`)
			.then((res) => {
				...
				for (let i = 0; i < res.data.length; i++) {
					if (
						res.data[i].MovementType === '201' ||
						res.data[i].MovementType === '221' ||
						res.data[i].MovementType === '261' ||
						res.data[i].MovementType === '281' ||
						res.data[i].MovementType === '101' ||
						res.data[i].MovementType === '105'
					) {
						// console.log('caught')
			...
			.catch((err) => {
				console.log(
					`error: there is some fetch error, check backend, error is: ${err.message}`
				);
			});
	}, [matcode]);
