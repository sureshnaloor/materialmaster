...
const ShowMaterialDetailssrch = (props) => {
	...
	useEffect(() => {
		...
		axios
			.get(`/api/matcodes/matcode/${matcode.toString()}`)
			.then((res) => {
	...
	useEffect(() => {
		// console.log(props.match.params.id)
		axios
			.get(`/api/valstock/mat/${matcode.toString()}`)
			.then((res) => {
	...
	useEffect(() => {
		console.log(props);
		axios
			.get(`/api/completestock/mat/${matcode.toString()}`)
			.then((res) => {
				setMatval(res.data.reduce((pr, cu) => pr + cu.ClosingValue, 0.0));
			})
			.catch((err) => {
				console.log(`error: there is some fetch error, check backend ${err}`);
			});
			...
			<div style={matcontainer}>
				<div style={cardMat}>
					<h5>
						{' '}
						Material:{' '}
						<span style={hilight}>{matcodes.MaterialDescription} </span>{' '}
					</h5>
					<h6>
						{' '}
						Material Code:{' '}
						<span style={hilight}> {matcodes.MaterialCode} </span>{' '}
					</h6>
					...
					<h6>
						...
						Active or closed:{' '}
						<span style={hilight}>
							{matcodes.closeFlag === 'NO' ? 'Active' : 'Closed'}{' '}
						</span>
					</h6>
				</div>
				<hr />
				<div style={cardMat2}>
					<h6>
						{' '}
						Total stock value of the material:{' '}
						<span style={hilight}>{matval} SAR</span>{' '}
					</h6>
					<h6>
						{' '}
						Special stock value of material out of above:{' '}
						<span style={hilight}>{matspval} SAR </span>
					</h6>
				</div>
			</div>
		</>
